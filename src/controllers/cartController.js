...
const cartCreation = async function (req, res) {
  try {
    ...
    if (!cartCheck) {
      ...
      let newCart = await (await cartModel.create(cartItems)).populate({ path: "items.productId" })
...
const updateCart = async function (req, res) {
  try {
    ...
    if (cartArray.length === 0) {
      return res.status(400).send({status:"cart is already empty."})
    }
...
const getCart = async function (req, res) {
  try {
    ...
    const cartCheck = await cartModel
      .findOne({ userId })
      .populate("items.productId");
    if (!cartCheck) {
      return res
        .status(404)
        .send({ status: false, message: "No cart found for this user" });
    }
...
const deleteCart = async (req, res) => {
  try {
    ...
    if (checkCart.items.length === 0 && checkCart.totalItems === 0 && checkCart.totalPrice === 0) {
