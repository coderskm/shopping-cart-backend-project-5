...
const cartCreation= async function(req,res){
    try{
        ...
        if(!cartCheck){
          ...
          const newCart = (await cartModel.create(cartItems)).populate('product')
          ...
          if(isAvilableProduct){
            const oldCart=await cartModel.findOneAndUpdate({_id:cartId,"items.productId":productId},{$inc:{totalPrice:+productCheck.price,"items.$.quantity": +1,}},{new:true}).populate('product')
              
            return res.status(201).send({status:true, data:oldCart})
        }else{
          const oldCart=await cartModel.findOneAndUpdate({_id:cartId},{$push:{items:{productId,quantity}},totalPrice:cartCheck.totalPrice+productCheck.price*quantity,totalItems:cartCheck.items.length+1},{new:true}).populate('product')
...
const getCart=async function(req,res){
    try{
        ...
        const cartCheck=await cartModel.findOne({userId}).populate('items.$*.product')
