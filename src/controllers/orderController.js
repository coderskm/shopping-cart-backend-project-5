...
const placeOrder=async function(req,res){
   try {
      ...
      const cartCheck = await cartModel.findOne({_id:cartId, userId:userId})
       if(!cartCheck){res.status(400).send({status:false,message:"cart does not belong to user"})}
...
const updateOrder=async function(req,res){
    try{
      ...
      if (!mongoose.isValidObjectId(orderId)) {
         return res.status(400).send({status:false, message:"orderId not valid"})
      }
    ...
    // if(checkOder.cancellable == false) {return res.status(400).send({status:false, message:"this oder can't be cancled"})}
       ...
       const orderData=await orderModel.findOneAndUpdate(orderId,{ status:status},{new:true})
